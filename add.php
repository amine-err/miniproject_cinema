        ...
        <div class="container">
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                <form method="post" action="add.php">
                    ...
                    <button type="submit" class="btn btn-success" name="sbmt">Ajouter un nouveau Film</button>
                </form>
            </div>
        </div>
    </div>
...
if (isset($_POST['sbmt'])) {
    try {
        ...
        $req = $conn->prepare("INSERT INTO films
        (poster,title,price,description,genre,year,duration,inProjection,director)
        VALUES (?,?,?,?,?,?,?,?,?)");
        ...
        $req->bindValue(8, $_POST['inProjection']);
        $req->bindValue(9, $_POST['director']);
        $req->execute();
        echo "Ajt bien ";
